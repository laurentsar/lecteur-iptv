...
| 📺 **En direct** | Chaînes en direct, recherche et filtre par catégorie/groupe (menu déroulant), badge « en cours / à suivre » quand un guide EPG est disponible. Bascule **Liste** (une chaîne par ligne) **/ Bouquets** (une grosse tuile par catégorie, icône selon le thème, toucher filtre dessus). Bouton **Picture-in-Picture** dans le lecteur pour continuer à regarder en mini-fenêtre. Les chaînes en double dans la playlist (sources/qualités multiples) sont **regroupées** sous une seule carte ; la lecture démarre directement sur la première source, les autres restent accessibles depuis le lecteur (bouton ⚙️ → Sources). |
